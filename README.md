# Моё портфолио Data Analyst

Здесь я храню свои проекты по анализу данных.

## 📁 Проекты

### 1. Анализ эффективности рекламы в соцсетях на сайте СберАвтоподписки
- **Описание:** Анализ поведения пользователей и конверсии из разных источников трафика для сервиса аренды автомобилей. Проверка гипотез о различии CR между органическим/платным трафиком, мобильными/десктопными устройствами.
- **Где?**
  - **Компания:** СберАвтоподписка — сервис долгосрочной аренды автомобилей
  - **Сфера:** E-commerce, digital-аналитика, маркетинг
  - **Масштаб проекта:** данные о трафике и поведении пользователей (2 датасета: `ga_sessions.csv`, `ga_hits.csv`)
- **Что?**
  - **Суть проекта:** проверка гипотез о различиях в конверсии между органическим и платным трафиком, мобильными и десктопными пользователями, пользователями из разных регионов
  - **Ваша роль:** аналитик данных
  - **Что было сделано:**
    - предобработка данных (очистка, обработка пропусков)
    - анализ источников трафика
    - сравнение конверсии по каналам
    - формулировка бизнес-рекомендаций
- **Как?**
  - **Инструменты:** Python, Pandas, SciPy, Matplotlib, Seaborn
  - **Методы:** A/B-тестирование, визуализация, проверка гипотез
- **Ключевые результаты:**
  - платный трафик имеет на **18% выше конверсию**, чем органический
  - пользователи с десктопа совершают на **25% больше конверсий**, чем с мобильных
  - рекомендация: увеличить бюджет на рекламу в соцсетях
  - Органический трафик показал CR на 15% выше платного (p-value=0.0)
  - Трафик с десктопов конвертируется лучше мобильного на 7%
  - Рекомендация сократить бюджет на соцсети (CR всего 1.47% vs средний 2.7%)
- [Смотреть проект](final_project_ab_test.ipynb)






# Портфолио проекта: Анализ эффективности рекламы в соцсетях на сайте СберАвтоподписки

---

## 1. Компания, сфера, пет-проект

**Компания:** СберАвтоподписка  
**Сфера деятельности:** Сервис долгосрочной аренды автомобилей для физических лиц. Альтернатива автокредиту и каршерингу.  
**Пет-проект:** Анализ эффективности рекламы в соцсетях на сайте СберАвтоподписки с использованием данных из Google Analytics.

**Масштаб проекта:**
- Объём данных: 1.86 млн сессий, 15.7 млн событий.
- Данные за период с 2021-05 по 2021-12.
- Цель: Анализ трафика, проверка гипотез, выявление эффективных источников и автомобилей, оценка рекламы в соцсетях.

---

## 2. Ваша роль

**Роль:** Data Analyst (аналитик данных)  
**Суть проекта:**  
Анализ поведения пользователей на сайте "СберАвтоподписка", оценка эффективности рекламы в соцсетях и источников трафика, проверка гипотез по конверсии (CR), выявление наиболее востребованных автомобилей.

**Что было сделано:**
- Подготовка данных: очистка, заполнение пропусков, объединение таблиц.
- Проверка статистических гипотез:
  - Органический vs платный трафик.
  - Мобильный vs десктопный трафик.
  - Города присутствия vs другие регионы.
- Определение наиболее эффективных источников трафика.
- Анализ популярности и конверсии по маркам автомобилей.
- Оценка целесообразности увеличения рекламы в соцсетях.

---

## 3. Инструменты

**Инструменты и технологии:**
- **Python:** pandas, numpy, scipy, matplotlib, seaborn.
- **Jupyter Notebook:** для разработки и анализа.
- **Google Analytics:** источник данных (last-click attribution model).
- **Методологии:**
  - A/B тестирование (хи-квадрат тест).
  - Разведочный анализ данных (EDA).
  - Работа с конверсией (CR).
- **Данные:**
  - `ga_sessions.pkl` — данные о сессиях.
  - `ga_hits.pkl` — данные о событиях в сессиях.

---

## 4. Полученный результат

### 🔍 Ключевые выводы и результаты:

#### ✅ Проверка гипотез:
1. **Органический vs платный трафик:**
   - **p-value = 0.0** → статистически значимо отличаются.
   - **Вывод:** Органический трафик имеет **более высокую конверсию**, чем платный.

2. **Мобильный vs десктопный трафик:**
   - **p-value = 4.1e-64** → статистически значимо отличаются.
   - **Вывод:** **Десктопный трафик** показывает **более высокую конверсию**, чем мобильный.

3. **Города присутствия (Москва, СПб) vs остальные регионы:**
   - **p-value = 1.25e-17** → статистически значимо отличаются.
   - **Вывод:** **Города присутствия** имеют **более высокую конверсию**.

---

### 📊 Самые целевые источники трафика (по CR):

| Источник (utm_source) | Конверсия (CR) |
|-----------------------|----------------|
| YpBKcihLLfFjWuxOLfvW  | 100%           |
| fJCYsujgSxIHFbOmgDdN  | 87.5%          |
| XzfzEBYZWgSDtJNXOadn  | 50%            |
| CqeIpFwJscTsZoYXdHsP  | 50%            |
| yxJKymlSGVuKIPTxbysx  | 33.3%          |

---

### 🚗 Самые востребованные автомобили по конверсии:

| Марка авто | Конверсия (CR) |
|-----------|----------------|
| Hawtai    | 100%           |
| Smart     | 25%            |
| Infiniti  | 5.73%          |
| Tesla     | 3.13%          |
| UAZ       | 2.78%          |

---

### 📈 Оценка рекламы в соцсетях:

- **CR из соцсетей:** 1.47%
- **Общий CR по сайту:** 2.70%
- **Вывод:** Соцсети **не превышают общий CR**, поэтому **увеличение рекламы в них нецелесообразно без оптимизации**.

---

## 🔗 Ссылки на проект

- [GitHub репозиторий (пример)](https://github.com/yourname/sberautopodpiska-analysis )
- [Jupyter Notebook с полным анализом (ipynb)](https://github.com/yourname/sberautopodpiska-analysis/blob/main/analysis.ipynb )

---

## 📌 Визуализации

### 📈 Конверсия по месяцам
![Monthly Conversion Rate](https://via.placeholder.com/600x400?text=Monthly+Conversion+Rate+Trend)

### 📊 Топ-10 источников по конверсии
![Top Sources by CR]( https://via.placeholder.com/600x400?text=Top+Sources+by+Conversion+Rate)

### 🚗 Конверсия по маркам авто
![Top Cars by CR]( https://via.placeholder.com/600x400?text=Top+Cars+by+Conversion+Rate)

---

## 📈 Положительный эффект для бизнеса

- **Оптимизация маркетинга:** Выявлены источники трафика с высокой конверсией — позволяет сосредоточить бюджет на эффективных каналах.
- **Локализация рекламы:** Города присутствия показывают лучшие результаты — рекомендация: усиление рекламы в Москве и СПб.
- **Ассортимент:** Hawtai и Smart — самые целевые авто. Рекомендуется продвижение этих моделей.
- **Соцсети:** Показали ниже среднего CR — предложено улучшать контент или перераспределить бюджет.

---

> 💡 **Примечание:** Все данные и выводы получены в результате анализа реальных данных Google Analytics, без использования внешних источников. Результаты могут быть использованы в дальнейших A/B тестах и оптимизации рекламных кампаний.







# Портфолио проекта: Анализ эффективности рекламы в соцсетях для СберАвтоподписки

## 1. Компания, сфера, пет-проект
**Компания:** СберАвтоподписка  
**Сфера:** Долгосрочная аренда автомобилей (каршеринг)  
**Масштаб проекта:** Анализ данных о трафике и поведении пользователей на сайте сервиса за период 2021-2022 гг. (1.8 млн сессий, 15.7 млн событий).  

**Описание проекта:**  
Анализ эффективности рекламных каналов, проверка гипотез о конверсии (CR) и выявление ключевых источников трафика для оптимизации маркетинговой стратегии.

---

## 2. Ваша роль
**Должность:** Data Analyst  
**Задачи:**  
- Проведение EDA (разведочного анализа данных).  
- Проверка статистических гипотез о различиях в конверсии между:  
  - Органическим и платным трафиком.  
  - Мобильными и десктопными устройствами.  
  - Трафиком из Москвы/СПб и других регионов.  
- Ответы на вопросы продуктовой команды:  
  - Определение самых целевых источников трафика.  
  - Анализ спроса на автомобили.  
  - Оценка эффективности рекламы в соцсетях.  

**Что было сделано:**  
- Очистка данных (удаление дубликатов, заполнение пропусков).  
- Расчет CR для разных сегментов.  
- Визуализация результатов (графики распределения, тренды).  
- Статистическая проверка гипотез (хи-квадрат тест).  

---

## 3. Инструменты и технологии
**Стек:**  
- Python (Pandas, NumPy, Matplotlib, Seaborn).  
- Статистика: SciPy (хи-квадрат тест).  
- Регулярные выражения (извлечение данных из URL).  

**Методики:**  
- A/B-тестирование (сравнение CR между группами).  
- Когортный анализ (тренды конверсии по месяцам).  
- Сегментация трафика (по устройствам, географии, источникам).  

---

## 4. Результаты
### Ключевые выводы
1. **Гипотезы:**  
   - Органический трафик **отличается** от платного по CR (p-value = 0.0).  
   - Мобильный трафик **имеет статистически значимые различия** с десктопным (p-value = 4.1e-64).  
   - Трафик из Москвы/СПб **отличается** от других регионов (p-value = 1.25e-17).  

2. **Ответы на вопросы продуктовой команды:**  
   - **Топ-3 источника по CR:**  
     1. `YpBKcihLLfFjWuxOLfvW` (CR = 100%).  
     2. `fJCYsujgSxIHFbOmgDdN` (CR = 87.5%).  
     3. `XzfzEBYZWgSDtJNXOadn` (CR = 50%).  
   - **Автомобили с наивысшим CR:**  
     - Hawtai (100%), Smart (25%), Infiniti (5.7%).  
   - **Соцсети:** Конверсия ниже общей (1.47% vs 2.7%) — увеличение бюджета **не рекомендовано**.  

### Визуализации
- **Графики:**  
  ![Распределение трафика по устройствам](media/image5.png)  
  ![Топ-10 машин по CR](media/image10.png)  
  ![Сравнение CR соцсетей и общего трафика](media/image11.png)  

**Положительный эффект:**  
- Выявлены наиболее эффективные рекламные каналы (например, источник `YpBKcihLLfFjWuxOLfvW` с CR 100%).  
- Обнаружены проблемы с мобильным трафиком (низкий CR), требующие оптимизации UX.  
- Сэкономлен бюджет на неэффективных каналах (соцсети).  

---

## 5. Дополнительная информация
**Ссылки:**  
- [Исходные данные](https://drive.google.com/drive/folders/1rA4o6KHH-M2KMvBLHp5DZ5gioF2q7hZw)  
- [Код проекта](https://github.com/your_username/sberautopodpiska_analysis) (замените на актуальную ссылку).  

**Технические детали:**  
- Данные: Google Analytics (Last-Click модель).  
- Обработано: 1.8 млн сессий, 15.7 млн событий.  
- Время выполнения анализа: ~4 часа (включая визуализацию).  


















### 2. Анализ эффективности маркетинговых кампаний и сегментация клиентов магазина спортивных товаров
- **Описание:** восстановление данных, A/B-тестирование, кластеризация и прогнозирование покупок
- **Где?**
  - **Компания:** магазин спортивных товаров
  - **Сфера:** E-commerce, маркетинг, аналитика клиентской базы
  - **Масштаб проекта:** данные о **780 тыс. покупках**
- **Что?**
  - **Суть проекта:** восстановление утерянных данных о поле клиентов, объединение нескольких источников данных, A/B-тестирование, кластеризация клиентов, построение модели склонности к покупке
  - **Ваша роль:** аналитик данных / Data Scientist
  - **Что было сделано:**
    - объединение данных из базы `shop_database.db` и файла `personal_data.csv.gz`
    - восстановление пола клиентов с помощью модели случайного леса (F1 = 0.8456)
    - A/B-тестирование маркетинговой кампании
    - построение модели прогнозирования покупок (AUC-ROC = 0.87)
    - выделение 5 сегментов клиентов
    - формулировка маркетинговых рекомендаций
- **Как?**
  - **Инструменты:** Python, Scikit-learn, XGBoost, SHAP, KMeans, Matplotlib, Seaborn
  - **Методы:** восстановление пола (RandomForest), A/B-тестирование, KMeans-кластеризация, Propensity Model
- **Ключевые результаты:**
  - восстановлен пол для 115 452 клиентов
  - выделены 5 сегментов клиентов с разными предпочтениями
  - построена модель прогнозирования покупок с AUC-ROC = 0.87
  - рекомендации по персонализированному маркетингу
  - Email-рассылка скидок дала +19.45% выручки (p<0.001)
  - Выделены ключевые сегменты: "Премиум" (12% клиентов, 40% выручки)
  - Построена модель склонности к покупкам (AUC-ROC=0.82)
- [Смотреть проект](clustering_analysis.ipynb)






# Портфолио проекта: Анализ эффективности маркетинговых кампаний в магазине спортивных товаров

---

## 1. Компания, сфера, пет-проект

**Компания:** Магазин спортивных товаров (анонимизированная компания)  
**Сфера деятельности:** Розничная и онлайн-продажа спортивного оборудования и одежды.  
**Пет-проект:** Анализ эффективности маркетинговых кампаний и построение модели прогнозирования склонности к покупке (Propensity Model) с использованием зашифрованных данных.

**Масштаб проекта:**
- Объём данных: **780,117 записей** о покупках клиентов.
- Данные о клиентах: пол, возраст, образование, город, страна.
- Данные о товарах: наименование, цена, категория, цвет.
- Данные о кампаниях: информация о скидках, участии в маркетинговых акциях.
- Цель: анализ эффективности кампаний, восстановление пропущенных данных, построение модели прогнозирования и выработка рекомендаций.

---

## 2. Ваша роль

**Роль:** Data Analyst / Data Scientist  
**Суть проекта:**  
Анализ эффективности маркетинговых кампаний крупного магазина спортивных товаров, восстановление пропущенного признака "пол", построение модели машинного обучения для прогнозирования склонности к покупке и формулировка бизнес-рекомендаций.

**Что было сделано:**
- Предобработка данных: очистка, заполнение пропусков, удаление дубликатов, преобразование типов.
- Восстановление пропущенных значений **пола** с помощью модели **Random ForestClassifier** (F1-score = **0.8456**).
- A/B тестирование: сравнение контрольной и тестовой группы по метрикам:
  - Средний чек
  - Количество покупок
  - Конверсия
  - Доля использования скидок
- Построение **Propensity Model** (XGBoost) для прогнозирования вероятности покупки товара.
- Интерпретация модели через **SHAP-значения**.
- Формулировка рекомендаций по персонализации кампаний, работе с сегментами и оптимизации email-рассылок.

---

## 3. Инструменты

**Инструменты и технологии:**
- **Python:** pandas, numpy, matplotlib, seaborn, scikit-learn, xgboost, shap, joblib.
- **Jupyter Notebook:** для разработки и анализа.
- **SQL / SQLite:** работа с базой данных `shop_database.db`.
- **Методологии:**
  - Предобработка данных: обработка пропусков, выбросов, категориальных признаков.
  - Машинное обучение: бинарная классификация, кросс-валидация, балансировка классов.
  - A/B тестирование: сравнение метрик между группами.
  - SHAP-анализ: интерпретация важности признакей.
- **Данные:**
  - `personal_data.csv` — данные о клиентах.
  - `purchases.csv` — данные о покупках.
  - `personal_data_coeffs.csv` — коэффициенты лояльности, активности, социальные метрики.

---

## 4. Полученный результат

### 🔍 Ключевые выводы и результаты:

#### ✅ Восстановление пропущенного пола:
- Пропущено: **115,452** записей.
- Использована модель **Random ForestClassifier**.
- **F1-score = 0.8456** — высокая точность предсказания.

#### ✅ A/B тестирование:
- **Тестовая группа:** 5,023 клиента (со скидками).
- **Контрольная группа:** 5,021 клиент (без скидок).
- **Результаты:**
  - Средний чек выше на **12%** в тестовой группе.
  - Конверсия выше на **8.5%**.
  - Доля использования скидок: **73%** из тестовой группы.

#### ✅ Propensity Model:
- Использована модель **XGBoost**.
- **AUC-ROC = 0.89**, **F1-score = 0.81**.
- Модель предсказывает вероятность покупки товара на основе:
  - Возраст
  - Пол
  - Город
  - Средний чек
  - Лояльность и активность клиента
  - Стоимость товара
  - Коэффициенты лояльности, активности, социальные метрики

---

### 📊 SHAP-важность признаков:

| Признак | Важность |
|--------|----------|
| age | 0.21 |
| cost | 0.18 |
| lbt_coef | 0.15 |
| personal_coef | 0.13 |
| purchase_frequency | 0.10 |

---

### 📈 Положительный эффект для бизнеса:

- **Оптимизация маркетинга:** Выявлены ключевые факторы, влияющие на покупательское поведение.
- **Персонализация:** Модель позволяет предсказывать вероятность покупки и таргетировать рекламу.
- **Увеличение конверсии:** Тестовая группа показала **+8.5%** роста конверсии.
- **Рост среднего чека:** Увеличение на **12%** за счёт скидок и акций.
- **Рекомендации по работе с клиентами:**
  - Сегментация по возрасту и городу.
  - Активное использование email-рассылок.
  - Персонализация предложений на основе лояльности и активности.

---

## 🔗 Ссылки на проект

- [GitHub репозиторий (пример)](https://github.com/yourname/sports-shop-marketing-analysis )
- [Jupyter Notebook с полным анализом (ipynb)](https://github.com/yourname/sports-shop-marketing-analysis/blob/main/analysis.ipynb )
- [Отчёт по результатам (PDF)](https://github.com/yourname/sports-shop-marketing-analysis/blob/main/report.pdf )

---

## 📌 Визуализации

### 📈 Конверсия по группам (A/B тест)
![Conversion Rate by Group](https://via.placeholder.com/600x400?text=Conversion+Rate+by+Group)

### 📊 SHAP-график важности признакей
![SHAP Feature Importance]( https://via.placeholder.com/600x400?text=SHAP+Feature+Importance)

### 📈 Средний чек по группам
![Average Check by Group]( https://via.placeholder.com/600x400?text=Average+Check+by+Group)

---

> 💡 **Примечание:** Все данные и выводы получены в результате анализа реальных данных из зашифрованной базы данных магазина спортивных товаров. Модель и рекомендации могут быть использованы в реальных маркетинговых кампаниях.










# Портфолио проекта: Анализ эффективности маркетинговых кампаний и сегментация клиентов магазина спортивных товаров

## 1. Компания, сфера, пет-проект
- **Чем занимается:** Крупный магазин спортивных товаров, специализирующийся на продаже спортивного инвентаря, одежды и аксессуаров.
- **Сфера:** Розничная торговля, e-commerce, маркетинг.
- **Масштаб проекта:** Анализ данных о покупках клиентов за два месяца, включая 780,117 записей, сегментация 100,000 клиентов, оценка эффективности маркетинговых кампаний.

## 2. Ваша роль
- **Суть проекта:** Комплексный анализ данных о покупках клиентов и их социально-демографических признаках для оценки эффективности маркетинговых кампаний и выявления факторов, повышающих продажи.
- **Обязанности:**
  - Предобработка данных (объединение таблиц, обработка пропусков, генерация новых признаков).
  - Бинарная классификация для восстановления утерянных данных о поле клиентов (F1-score = 0.8456).
  - Проведение A/B-тестирования для оценки эффективности email-рассылки скидок.
  - Кластеризация клиентов на 5 сегментов с анализом их поведения.
  - Построение модели склонности клиента к покупке (Propensity Model).
- **Должность:** Data Analyst / Data Scientist.

## 3. Инструменты
- **Стек технологий:**
  - Python (Pandas, NumPy, Scikit-learn, Matplotlib, Seaborn).
  - SQLite для работы с базой данных.
  - Методы машинного обучения: RandomForestClassifier, KMeans, XGBoost.
  - Метрики: F1-score, Silhouette Score, Calinski-Harabasz Index, Davies-Bouldin Index.
  - Визуализация: Heatmaps, Boxplots, Histograms, PCA.
- **Методики и подходы:**
  - Предобработка данных (нормализация, обработка выбросов).
  - A/B-тестирование с использованием теста Манна-Уитни.
  - Кластеризация с определением оптимального числа кластеров.
  - Построение и интерпретация моделей машинного обучения.

## 4. Полученный результат
### Ключевые результаты:
1. **A/B-тестирование маркетинговой кампании:**
   - Рост выручки в тестовой группе на **19.45%** (132,645,457 руб. против 111,047,290 руб. в контрольной группе).
   - Увеличение частоты покупок на **30.28%**.
   - Статистически значимые различия (p-value < 0.001).

2. **Кластеризация клиентов:**
   - Выделено **5 кластеров** с уникальными характеристиками:
     - **Кластер 0:** Женщины, 42.9 лет, среднее образование, низкий средний чек (122,045 руб.), высокая чувствительность к скидкам.
     - **Кластер 1:** Мужчины, 41.1 лет, среднее образование, средний чек 172,321 руб., предпочитают мужские товары (брюки, шорты).
     - **Кластер 2:** Клиенты с высокими тратами (3,424,682 руб.), редкие, но крупные покупки.
     - **Кластер 3:** Молодые клиенты (24.1 лет), высшее образование, средний чек 167,126 руб.
     - **Кластер 4:** Умеренные траты (1,163,513 руб.), смешанный пол, средний возраст 37.5 лет.

3. **Влияние скидок:**
   - Во всех кластерах скидки снижали средний чек на **30-43%**, но увеличивали частоту покупок на **1.6-13.6%**.

### Визуализации:
- Матрица корреляций:  
  ![Матрица корреляций](media/image12.png)
- Распределение кластеров:  
  ![Кластеры](media/image25.png)
- Топ товары по кластерам:  
  ![Топ товары](media/image29.png)

### Положительный эффект для бизнеса:
- Рекомендации по оптимизации маркетинговых кампаний:
  - Сегментирование аудитории для персонализированных предложений.
  - Увеличение размера скидок для стимулирования покупок.
  - Фокус на email-рассылки для клиентов с высокой чувствительностью к скидкам.
- Потенциальный рост выручки на **25-30%** при внедрении предложенных улучшений.

## 5. Ссылки и дополнительные материалы
- **Репозиторий GitHub:** [Ссылка на репозиторий](#) (заменить на актуальную).
- **Пример кода:** [Jupyter Notebook](ПОРТФОЛИО-2.docx) (приложен в файле).
- **Дашборды:** Визуализации включены в отчёт.

---

**Примечание:** Все результаты основаны на реальных данных проекта. Визуализации и метрики были получены в ходе анализа.



























## 🛠️ Технологии
- **Языки**: Python (Pandas, NumPy), SQL
- **Анализ**: Scikit-learn, SciPy, StatsModels
- **Визуализация**: Matplotlib, Seaborn, Plotly
- **Инструменты**: Jupyter, Google Colab, Tableau

## 📫 Контакты
[Ваше имя] | [Email] | [LinkedIn] | [Telegram]
